...
public class GraficaMain extends JFrame {
	...
	/**
	 * Elementos que utilizaremos en la partida Imagen offGame (Cubre todo) Estado
	 * inGame Partida actual Personaje objetivo Cronometro Controlador de Cronometro
	 * Representacion del cronometro Lista de todos los elementos existentes en el
	 * juego (Son pasados a la mezcladora) Mezcladora del juego Lista de los
	 * elementos iniciales y desbloqueados en juego (Van a las listas 1 y 2) Lista
	 * izquierda Lista derecha Resultado de la mezcla Boton de combinar Imagen del
	 * personaje objetivo Audio: Musica boolean: musicOn Audio: efecto 1,2 y 3
	 * Manejadora de archivos Rutas de archivos: compuestos,iniciales y personajes
	 */
	...
	private static ListaDeElementos listaDeElementos; // Lista de todos los elementos existentes en el juego (Son
														// pasados a la mezcladora)
	private static Mezcladora mezcladora; // Mezcladora del juego
	private static ListaDisponibles listaDisponibles; // Lista de los elementos iniciales y desbloqueados en juego (Van
														// a las listas 1 y 2)
	...
	public GraficaMain() {
		...
		efectoExistente = java.applet.Applet
				.newAudioClip(getClass().getResource("/appSonidos/combinacionExistente.wav"));
		...
		*/
		...
		try {
			...
			System.out.println("No se pudo cargar imagen: backText");
		}
	...
	public static void nuevaPartida() throws NoPartidaException{
		...
		if (nuevaPartida.getPartidaNueva() != null) {
			...
			// En caso de que esta sea la segunda partida creada, cerramos el hilo del
			// cronometro de la primera
			...
			System.out.println(
					partida.getNombreJugador() + " con personaje objetivo: " + partida.getNombrePersonajeObjetivo());
			// Quitamos el cover
			cover.setVisible(false);
			btnCombinar.setVisible(true);

			// Adaptacion para interfaz Swing, cerrando el cronometro existente y utilizar
			// el de la interfaz grafica para controlar
			...
			inGame = true;
			try {
				imagenPersonaje.setIcon(new ImageIcon(GraficaMain.class
						.getResource("/appImagenes/" + partida.getNombrePersonajeObjetivo() + ".jpg")));
			} catch (Exception e) {
				imagenPersonaje
	...
	/**
	 * Combina los elementos seleccionados y llama a funcion ganar en caso de su
	 * resultado ser el objetivo
	 */
	...
	/**
	 * Termina la partida y en caso de ser un nuevo tiempo record, registra el
	 * tiempo nuevo del personaje
	 */
	public static void partidaGanada() {
		hiloCronometro.close();
		partida.setPuntajeJugadorPorCronometro();
		System.out.println("Ganaste!, tu puntuacion fue de: " + partida.getPuntajeJugador() + " con tiempo de: "
				+ partida.getTimer());
		cover.setVisible(true);
		partidaGanada.play();
